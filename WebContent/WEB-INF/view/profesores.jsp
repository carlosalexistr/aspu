					...
					<div class="container-fluid flex-grow-1 container-p-y">
						<h4
							...
							<ol class="breadcrumb font-weight-bold mb-3">
								...
								<li class="breadcrumb-item"><a href="javascript:void(0)">Profesores</a>
								</li>
							</ol>
							...
							<form action="profesores" method="post">
								<div class="row">
									...
									<div class="col-2">
										<button type="submit"
											class="btn icon-btn btn-outline-dark"> <span
						...
						<c:forEach var="item" items="${requestScope.profesores}">
						  <div class="card">
								...
								<div class="card-body">
									<p class="card-text">
										<small class="text-muted"><c:out value="${item.fechaRegistro}"/></small>
									</p>
									<h4 class="card-title"><c:out value="${item.nombres} ${item.apellidos}"/></h4>
									<div class="card-text"><c:out value="Mi nombre es ${item.nombres} ${item.apellidos}, soy Docente con el titulo de ${item.especializacion}"/></div>
									<p class="card-text">
										<a href="profesor?id=${item.id}"
											class="card-footer d-block text-center text-dark small font-weight-semibold">VER MAS</a>
									</p>
								</div>
							</div>
						</c:forEach>
